...
void gpio_init(void)
{
	...
	/* Each GPIO has 3 bits which determine its function
	 * GPIO 22 is in GPFSEL2
	 */
	/* Read current value of GPFSEL2 */
	var = *gpioGPFSEL2;
	...
	*gpioGPFSEL2 = var;
	...
	/* Set up pull-down on GPIO22 */
	/* Enable pull-down control, then wait at least 150 cycles
	 * The delay loop actually waits longer than that
	 */
	*gpioGPPUD = 1;
	delay();

	/* Set the pull up/down clock for pin 22*/
	*gpioPUDCLK0 = 1<<22;
	*gpioPUDCLK1 = 0;
	delay();
